...
<form id="formulario" method="post" action="/derrame/{{$derrame->id}}">
		...
		<div class='col-md-12'>
			<div class="form-group">
				<div class="input-group date" id="datetimepicker3">
					...
					<textarea required="" id="pinformacion_inicial" class="form-control" maxlength="2000" name="informacion_inicial" aria-label="With textarea">{{old('informacion_inicial',$derrame->informacion_inicial)}}</textarea>
				</div>
			</div>
		</div>
	</div>
	...
	<div class="form-row"><!--Div Ubicacion Evento-->
		<div class="form-group input-group col-md-6">
			...
			<!-- onkeyup="mayus(this);" -->
			<textarea required="" class="form-control" id="pdireccion" name="address" placeholder="Ubicacion del Evento" aria-label="With textarea">{{old('address',$derrame->direccion)}}</textarea>
	...
	<div class="form-row"><!--Div Personal que asiste Evento-->
		<div class="form-group input-group col-md-6">
			...
			<select required class="selectpicker form-control" data-live-search="true" name="jefeguardia_id">
				@if($derrame->users->count() > 2)
					<option value="{{ $derrame->users[2]->id }}" selected>
						{{ $derrame->users[2]->name }}
					</option>
				@else
				...
				@foreach($bomberos as $bombero)
					<option value="{{ $bombero->id }}"
						{{ old('jefeguardia_id') == $bombero->id ? 'selected' : '' }}>
						{{ $bombero->name }}
					</option>
				@endforeach
			</select>
	...
	<div class="form-row">
		<div class="col-lg-12 col-md-12 col-sm-12">
			<div class="form-group input-group">
				...
				<textarea required="" class="form-control" name="detalle_emergencia" id="detalle_emergencia" maxlength="3000" aria-label="With textarea" placeholder="Digite a detalle lo ocurrido en Emergencia">{{old('detalle_emergencia',$derrame->detalle_emergencia)}}</textarea>
			</div>
		</div>
				...
				<div class="col-lg-12 col-sm-12 col-md-12 col-xs-12">
					<table id="detalles" class="table table-striped table bordered table condensed table-hover">
						...
						<tr class="selected" id="fila{{count($derrame->vehiculos)}}">
							...
							<td><input class="form-control" type="number" name="driver_id[]" value="{{$vehiculo->pivot->driver_id}}">{{$vehiculo->pivot->driver_id}}</td>
						</tr>
						<tbody></tbody>
						@endforeach
					</table>
