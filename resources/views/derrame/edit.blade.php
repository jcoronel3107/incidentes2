	...
	@section( "cuerpo" )
		<h2 class="mt-5 shadow p-3 mb-5 bg-white rounded text-danger">Edita Información de Eventos Derrame</h2>
		<ul class="nav justify-content-end">
			<li class="nav-item">
				<a class="btn btn-outline-info" data-toggle="tooltip" title="Cancel" role="button" href="{{ route('derrame.index')}}"><i class="fa fa-arrow-left icon-2x" aria-hidden="true"></i>
				</a>
			</li>
		</ul>
		<hr style="border:2px;">
		...
		<form id="formulario" method="post" action="/derrame/{{$derrame->id}}">
				...
				<div class='col-md-10'>
					<div class="form-group">
						<div class="input-group">
							...
							<input onkeyup="mayus(this);" type="text" name="ficha_ecu911" value="{{old('ficha_ecu911',$derrame->ficha_ecu911)}}" class="form-control">
						</div>
					</div>
				</div>
			</div>
				...
				<div class='col-md-12'>
					<div class="form-group">
						<div class="input-group date">
							...
							<textarea class="form-control" maxlength="2000" name="informacion_inicial" id="pinformacion_inicial" aria-label="With textarea">{{old('informacion_inicial',$derrame->informacion_inicial)}}</textarea>
						</div>
					</div>
				</div>
			</div>
			...
			<div class="form-row"><!--Div  Direccion-->
					...
					<div class="row">
							...
							<table id="persontable" class="table table-hover">
								...
								<thead>
										...
										@foreach($derrame->users as $users)
					...
					<div class="row d-flex ">
							<div class="col-lg-12 col-sm-12 col-md-12 col-xs-12">
								<table id="detalles" class="table table-hover">
									...
									<tbody>
										{{$cont = 0}}
										@foreach($derrame->vehiculos as $items)
										{{$cont = $cont + 1}}
										<tr id="fila{{$cont}}">
			...
			<div class="form-row"><!--Div Horas Evento-->
				<div class="form-group  input-group col-md-12">
					...
					<input type="datetime-local" class="form-control" name="hora_salida_a_emergencia" id="hora_salida_a_emergencia" value="{{old('hora_salida_a_emergencia',$derrame->hora_salida_a_emergencia)}}" onblur="CheckTime(this);" placeholder="hh:mm:ss" required="">
				...
				<div class="form-group  input-group col-md-12">
					...
					<input type="datetime-local" class="form-control" name="hora_llegada_a_emergencia" id="hora_llegada_a_emergencia" value="{{old('hora_llegada_a_emergencia',$derrame->hora_llegada_a_emergencia)}}" placeholder="hh:mm:ss" onblur="CheckTime(this);" required="">
					<div class="input-group-append">
			...
			<div class="form-row"><!--Div Horas Evento-->
				<div class="form-group  input-group col-md-12">
					...
					<input type="datetime-local" class="form-control" name="hora_fin_emergencia" value="{{old('hora_fin_emergencia',$derrame->hora_fin_emergencia)}}" id="hora_fin_emergencia" onblur="CheckTime(this);" placeholder="hh:mm:ss">
					<div class="input-group-append">
				...
				<div class="form-group  input-group col-md-12">
					...
					<input type="datetime-local" class="form-control" name="hora_en_base" value="{{old('hora_en_base',$derrame->hora_en_base)}}" id="hora_en_base" onblur="CheckTime(this);" placeholder="hh:mm:ss">
					<div class="input-group-append">
			...
			<div class="form-row"><!-- Detalle Emergencia -->
				...
				<div class="col-lg-12 col-md-12 col-sm-12">
					<div class="form-group input-group">
						...
						<textarea class="form-control" maxlength="3000" name="detalle_emergencia" id="detalle_emergencia" aria-label="With textarea" placeholder="Digite a detalle lo ocurrido en Emergencia">{{old('detalle_emergencia',$derrame->detalle_emergencia)}}</textarea>
					</div>
				</div>
			</div>
			...
			<div class="form-row"><!-- Usuario Afectado  -->
				<div class="col-12">
					<div class="form-group input-group">
						...
						<input onkeyup="mayus(this);" type="text" class="form-control" name="usuario_afectado" id="usuario_afectado" value="{{old('usuario_afectado',$derrame->usuario_afectado)}}" placeholder="Digite Nombre Completo ciudadano afectado en la Emergencia">
					</div>
				</div>
			</div>
			...
			<div class="form-row"><!-- Daños Estimados  -->
				<div class="form-group input-group col-lg-12 col-md-12 col-sm-12">
					...
					<textarea onkeyup="mayus(this);" class="form-control" name="danos_estimados" id="danos_estimados">{{old('danos_estimados',$derrame->danos_estimados)}}</textarea>
				</div>
			</div>
			...
			<div class="form-group"><!-- Botones  -->
				...
				<a class="btn btn btn-primary" role="button" href="{{ route('derrame.index')}}">Cancelar
				</a>
			</div>
		</form>
		<form method="post" action="/derrame/{{$derrame->id}}"> <!-- Form Eliminar -->
			{{csrf_field()}}
